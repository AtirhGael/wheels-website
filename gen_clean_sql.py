...
with open('/opt/lampp/htdocs/elitebbs/seed_clean.sql', 'w') as out:
    out.write("-- Elite BBS Rims - Seed File (809 products)\n")
    ...
    for i, prod in enumerate(all_products):
        ...
        sql = f"INSERT IGNORE INTO products (name, slug, short_description, description, price, sale_price, sku, stock, category, brand, size, finish, images, featured, status) VALUES ({escape_sql(name)}, {escape_sql(slug)}, {escape_sql(short)}, {escape_sql(long)}, 0, 0, {escape_sql(sku)}, 10, {escape_sql(cat)}, {escape_sql(brand)}, {escape_sql(size)}, {escape_sql(finish)}, '[]', 0, 'active');\n"
        
        out.write(sql)
    
    out.write("SET FOREIGN_KEY_CHECKS = 1;\n")

print("Done! Writing seed_clean.sql")